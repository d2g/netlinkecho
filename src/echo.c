...
struct sock *netlink_echo_socket = NULL;
...
int init_nlecho() {
    ...
    netlink_echo_socket = netlink_kernel_create(&init_net, NETLINK_CHANNEL, &netlink_config);
    if(!netlink_echo_socket)
    {
...
int dest_nlecho(){
    netlink_kernel_release(netlink_echo_socket);
    return 0;
}

void nlecho(struct sk_buff *netfilter_socket_buffer){
    ...
    res=nlmsg_unicast(netlink_echo_socket,response,senders_pid);
    if(res < 0) {
